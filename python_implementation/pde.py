...
class PDE(Data):
    ...
    def __init__(
        ...
        self.idx_pde_prec = 0 # used to save the index of the previous final element of the batch
        self.idx_bc_prec = 0
        self.epoch_seed = 123
        self.num_bcs_batch = None
    ...
    def train_next_batch(self, batch_size=None):
        ...
        if batch_size is not None:
            ...
            if batch_size > len_training:
                warnings.warn(f'The given batch_size ({batch_size}) is bigger than the full training set so is set as full batch: {len_training}')
                batch_size = len_training

            batch_size_bc = max(len(self.bcs),int(batch_size * len_bc/len_training))
            batch_size_pde = batch_size - batch_size_bc

            bc_end = min(self.idx_bc_prec + batch_size_bc, len_bc)
            pde_end = min(self.idx_pde_prec + batch_size_pde, len_pde)

            bc_batch = self.train_x_bc[self.idx_bc_prec: bc_end]
            pde_batch = self.train_x_all[self.idx_pde_prec : pde_end]

            # Combine batches (BCs first, then PDE points) so that losses()
            # can still assume this ordering.
            self.train_x = np.vstack((bc_batch, pde_batch))

            # Update indices for next batch
            self.idx_bc_prec = bc_end
            self.idx_pde_prec = pde_end

            if self.idx_bc_prec >= len_bc or self.idx_pde_prec >= len_pde:
                rng = np.random.default_rng(self.epoch_seed)
                self.train_x_bc = self.train_x_bc[rng.permutation(len_bc)]
