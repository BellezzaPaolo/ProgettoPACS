...
# losshistory, train_state = model.train(iterations = budget, batch_size=10, display_every = 5, callbacks = [dde.callbacks.BudgetCallback(budget)])
...
losshistory, train_state = model.train(iterations = budget, batch_size = 10, display_every=int(1e5//100), callbacks = [dde.callbacks.BudgetCallback(budget)])
dde.saveplot(losshistory, train_state, issave=True, isplot=True)
